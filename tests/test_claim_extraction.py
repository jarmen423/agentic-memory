...
def _mock_response(payload: str) -> Mock:
    """Build a Groq-like response wrapper for a JSON payload string."""
...
def test_extract_returns_normalized_claims():
    ...
    with patch("codememory.core.claim_extraction.Groq", Mock(return_value=mock_client)):
...
def test_extract_truncates_input_to_8000_characters():
    """Only the first 8000 characters are sent to Groq."""
    mock_client = Mock()
...
def test_extract_wraps_groq_errors():
    """Groq client failures are re-raised as RuntimeError."""
    mock_client = Mock()
...
def test_extract_uses_json_object_mode_and_zero_temperature():
    """The Groq request uses deterministic json_object mode."""
    mock_client = Mock()
    mock_client.chat.completions.create.return_value = _mock_response('{"claims":[]}')
